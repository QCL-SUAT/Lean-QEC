    ...
    · intro j
      ...
      · refine BitVec.eq_of_getElem_eq ?_
        ...
        have hlow : (↑j * Nat.clog 2 n + bit) < i.succ * Nat.clog 2 n := by
          nlinarith [j.2, hbit]
        simp [hlow]
        have hidx : (j.castSucc : Fin (i + 1 + 1)) = ⟨↑j, by simp⟩ := rfl
        have hrec := congrArg (fun bv : BitVec (Nat.clog 2 n) => bv[bit])
          (ih (r := fun i : Fin (i + 1) => r ⟨i, by simp⟩) j)
        rw [hidx]
        simp only [BitVec.getElem_extractLsb' hbit, BitVec.natCast_eq_ofNat] at hrec
        convert hrec using 1
        exact (BitVec.getLsbD_eq_getElem
          (x := (vec_to_BitVec' fun i_1 : Fin (i + 1) => r ⟨i_1.1, by omega⟩)) hlow).symm
...
lemma linear_indep_of_forall_vecmul_nz {r n : ℕ} (M : Matrix (Fin r) (Fin n) (ZMod 2))
  : LinearIndependent (ZMod 2) M ↔ ∀ coeffs, coeffs ≠ 0 → M.vecMul coeffs ≠ 0 := by
    rw [show LinearIndependent (ZMod 2) M ↔
        ∀ (g : Fin r → ZMod 2), (∑ i, g i • M i) = 0 → ∀ i, g i = 0 from
