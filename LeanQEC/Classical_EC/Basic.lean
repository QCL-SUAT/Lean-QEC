...
theorem generatorMatrix_image_eq_CodeSpace (C : CodeSpace n) (G : Matrix α (Fin n) (ZMod 2))
  ...
  congr
  rw [show (⟨G a, hw⟩ : ↥(Set.range ⇑mmap)) = ⟨mmap a, Set.mem_range_self a⟩ from rfl]
  exact mmap.right_inv_of_invOfMemRange a
...
theorem genMatrix_size_eq {k : ℕ} (C : CodeSpace n) (G : Matrix (Fin k) (Fin n) (ZMod 2))
  ...
  have h : Set.range (fun r => G r) = (Finset.image (fun r => G r) Finset.univ) := by
    ext w
    simp only [Set.mem_range, Finset.mem_coe, Finset.mem_image, Finset.mem_univ, true_and]
  rw [h] at this
  exact this
