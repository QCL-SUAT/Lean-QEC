...
theorem Pauli_to_BinSymp_correct {n : ℕ} (P₁ P₂ : PauliGroup_group n) :
  ...
  · have hi : (foldPauli.symm (P₁ * P₂)).2 i = (mul_Pauli1 (FP₁.2 i) (FP₂.2 i)).2 := by
      simpa [FP₁, FP₂, mul_factored_Paulis, scale_factored_Pauli, mul_Pauli1_tuples,
        collect_phases, pointwise_mul_Paulis] using
...
lemma BinSympMatrix.rowProduct_binaryImage {n k : ℕ} (bsm : BinSympMatrix k n) (h_comm : bsm.isCommuting)
  ...
  have h_sum : ∀ (ind : Fin k → ZMod 2),
    ...
    · have h1 : bsm.rowProduct h_comm (0 : Fin k → ZMod 2) = 1 :=
        BinSympMatrix.rowProduct_zero bsm h_comm
      rw [h1, Pauli_toBinSympPauli_one]
      simp
    · have h_sum :
...
lemma StabCode.finrank_normalizerSpace {n : ℕ} (C : StabCode n) :
  ...
  rw [StabCode.normalizerSpace,
    LinearMap.BilinForm.finrank_orthogonal (BinSympPauli.form_nondegenerate n)
      C.stabSymplecticSpace,
    StabCode.numGenerators, BinSympPauli.finrank]
