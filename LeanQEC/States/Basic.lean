...
@[simp] lemma qub_zero_one : qub_zero 1#1 = 0 := by
  simp [qub_zero, beq, BitVec.equivFin]
...
lemma ket_prodE {n₁ n₂} (ψ₁ : BitVec n₁ → ℂ) (ψ₂ : BitVec n₂ → ℂ) :
  ...
  λ(s : BitVec (n₁ + n₂)) =>
    ...
    ψ₁ s₁ * ψ₂ s₂ := by
  ext s
  simp [ket_prod, toMat_symm_apply', toMat_apply', normalized_kron, Matrix.kronecker,
    Matrix.kroneckerMap_apply, Matrix.reindex_apply, Matrix.submatrix_apply]
