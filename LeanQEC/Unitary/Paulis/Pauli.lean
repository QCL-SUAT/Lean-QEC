...
-- v4.34: `instance` requires a type-class return type; a bare `Subgroup` value is not one,
-- so this is an error now ("should not be an instance as its return type ... is not a type class").
-- Keep `@[reducible]` so that downstream rewriting still sees through the definition
-- (the old `instance` was unfolded at instance transparency).
@[reducible] def PauliGroup_group (n : ℕ) : Subgroup 𝐔ₙ[n] where
